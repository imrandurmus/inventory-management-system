...
const RDashboard: React.FC = () => {
  ...
  const [sortQuantityOrder, setSortQuantityOrder] = useState<
    "asc" | "desc" | null
  >(null);
    ...
    const fetchData = async () => {
      try {
        ...
        const inventoryItems: InventoryItem[] = productData.products
          .filter((p) =>
            employeeData.assignedItemTypes.includes(p.productType.name)
          )
          .map((p) => ({
            id: p.id,
            name: p.name,
        ...
        const prices = productData.products.reduce(
          (acc, p) => ({
            ...acc,
            [p.id]: p.price,
          }),
        ...
        setError(
          err.message ||
            "Failed to load dashboard. Please try again or contact tech support."
        );
      } finally {
        setLoading(false);
      }
  ...
  if (stockFilter) {
    filteredInventory = filteredInventory.filter((item) => {
      if (stockFilter === "Low Stock") {
        return (
          item.quantityInStock <= item.reorderLimit && item.quantityInStock > 0
        );
  ...
  const confirmRestock = async () => {
    if (restockItem && restockQuantity) {
      ...
      try {
        ...
        const { products } = await getProducts(0, 1000, "id");
        const productData = products.find((p) => p.id === restockItem.id);
        ...
        setTotalStockValue(
          totalStockValue + quantity * (productPrices[restockItem.id] || 100)
        );
        setLowStockItems(
          inventory
            .map((item) =>
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `inventory_${employee?.name || "employee"}_${
        new Date().toISOString().split("T")[0]
      }.csv`
    );
